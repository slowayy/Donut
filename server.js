...
app.get("/discord-user/:userId", async (req, res) => {
  ...
  try {
    ...
    if (member.presence) {
      ...
      userData.activities = member.presence.activities
        .map((activity) => {
          if (activity.type === 2 && activity.name === "Spotify") {
            ...
            userData.spotify = {
              ...
              album_cover_url: activity.assets
                ? `https://i.scdn.co/image/${activity.assets.largeImage.slice(8)}`
                : null,
              track_url: activity.syncId
          ...
          return {
            ...
            large_image_key: activity.assets
              ? activity.assets.largeImage
              : null,
            small_image_key: activity.assets
              ? activity.assets.smallImage
              : null,
            large_image_text: activity.assets
              ? activity.assets.largeText
              : null,
            small_image_text: activity.assets
